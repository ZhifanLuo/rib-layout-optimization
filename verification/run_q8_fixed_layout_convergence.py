"""Run fixed-layout convergence with the independent Q8 shell reference."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--meshes", default="20x10,40x20,80x40,160x80,320x160",
    )
    parser.add_argument("--interface-mesh", default="80x40")
    parser.add_argument("--interface-levels", default="2,4,8,16")
    parser.add_argument("--include-no-rib", action="store_true")
    ...
    from rib_layout_algorithms.model_shell_q8 import (
        Q8EmbeddedLineBeamShellReferenceModel,
    )
    ...
    def run_series(meshes: list[tuple[int, int]], with_ribs: bool) -> list[dict]:
        rows = []
        for nx, ny in meshes:
            started = time.perf_counter()
            model = Q8EmbeddedLineBeamShellReferenceModel(
                40.0, 20.0, nx, ny, **common,
            )
            response = model.analyze(
                ribs if with_ribs else [], thicknesses if with_ribs else []
            )
            rows.append({
                "nx": nx, "ny": ny, "nnode": model.nnode, "ndof": model.ndof,
    ...
    ribbed = run_series(meshes, True)
    no_rib = run_series(meshes, False) if args.include_no_rib else []
    interface_nx, interface_ny = parse_meshes(args.interface_mesh)[0]
    interface_scan = []
    for level in [int(value.strip()) for value in args.interface_levels.split(",")]:
        started = time.perf_counter()
        model = Q8EmbeddedLineBeamShellReferenceModel(
            40.0, 20.0, interface_nx, interface_ny,
            **{**common, "interface_subdivisions_per_cell": level},
        )
    ...
    payload = {
        ...
        "coupling": "q8_shell_embedded_consistent_line_beam",
        "case": 2,
